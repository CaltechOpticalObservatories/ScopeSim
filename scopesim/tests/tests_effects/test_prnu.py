"""Tests for pixel response non-uniformity."""
...
def make_detector(value=1000, size=10):
    hdr = header_from_list_of_xy([-size / 2, size / 2],
                                 [-size / 2, size / 2], 1, "D")
    dtcr = Detector(hdr)
...
def test_output_std_matches_prnu_std():
    prnu_std = 0.05
